...
;Version Information
  ...
  VIAddVersionKey /LANG=${LANG_ENGLISH} "ProductName" "LC2JAVA_HOME"
  ...
  VIAddVersionKey /LANG=${LANG_ENGLISH} "FileDescription" "LC2JAVA_HOME"
...
Name "LC2JAVA_HOME"
Caption "LC2JAVA_HOME Install (c)by http://www.letztechance.org"
Icon "${NSISDIR}\Contrib\Graphics\Icons\lc.ico"
OutFile "LC2JAVA_HOMEinstall.exe"
...
InstallDir "$PROGRAMFILES\LETZTECHANCE.ORG\LC2JAVA_HOME"
InstallDirRegKey HKLM "Software\LETZTECHANCE.ORG\LC2JAVA_HOME" "LC2JAVA_HOME"
...
;	CreateShortCut "$SMPROGRAMS\$R0\All users LC2JAVA_HOME.lnk" $INSTDIR\LC2JAVA_HOME.exe
;Abort
;!else
...
;MessageBox MB_YESNO "Wollen Sie dieses Programm wirklich installieren ?" IDYES
  ...
  WriteRegStr HKLM SOFTWARE\WWW.LETZTECHANCE.ORG\LC2JAVA_HOME "Install_Dir" "$INSTDIR"

  ; write uninstall strings
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\LC2JAVA_HOME" "DisplayName" "LC2JAVA_HOME (remove only)"
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\LC2JAVA_HOME" "UninstallString" '"$INSTDIR\LC2JAVA_HOMEuninst.exe"'
;---Extrahiere Dateien --------------------->
  SetOutPath $INSTDIR
  ;File /a "exe\LC2JAVA_HOME.exe"
  File /a "exe\*.*"
...
;  File /a "exe\WebSearch\*.*"
  ...
  WriteUninstaller "LC2JAVA_HOMEuninst.exe"
;---Extrahiere Bilder --------------------->
...
SectionIn 1 4 3
  ...
  WriteRegStr HKLM SOFTWARE\LETZTECHANCE.ORG\LC2JAVA_HOME "StrTest_INSTDIR" "$INSTDIR"
  WriteRegDword HKLM SOFTWARE\LETZTECHANCE.ORG\LC2JAVA_HOME "DwordTest_0xDEADBEEF" 0xdeadbeef
  WriteRegDword HKLM SOFTWARE\LETZTECHANCE.ORG\LC2JAVA_HOME "DwordTest_123456" 123456
  WriteRegDword HKLM SOFTWARE\LETZTECHANCE.ORG\LC2JAVA_HOME "DwordTest_0123" 0123
  WriteRegBin HKLM SOFTWARE\LETZTECHANCE.ORG\LC2JAVA_HOME "BinTest_deadbeef01f00dbeef" "DEADBEEF01F00DBEEF"
;  StrCpy $8 "$SYSDIR\LC"
...
Section "CreateShortCuts"
 ...
 CreateDirectory $SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2JAVA_HOME
 ;CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2JAVA_HOME\Start LC2JAVA_HOME.lnk" $INSTDIR\start.bat
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2JAVA_HOME\LC2JAVA_HOME.lnk" $INSTDIR\LC2JAVA_HOME.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2JAVA_HOME\Update.lnk" $INSTDIR\LC2Update.exe
 ;CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2JAVA_HOME\LC2JAVA_HOME.exe" $INSTDIR\LC2JAVA_HOME.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2JAVA_HOME\LC2JAVA_HOMEuninst.lnk" $INSTDIR\LC2JAVA_HOMEuninst.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2JAVA_HOME\Support.lnk" $INSTDIR\letztechance.org.url
  SectionIn 1 2 3
...
Section "Programm und Hilfe ausführen..." TESTIDX
  ...
  SearchPath $1 $INSTDIR\LC2JAVA_HOME.exe
...
UninstallText "Uninstall LC2JAVA_HOME ?"
;UninstallIcon "${NSISDIR}\Contrib\Graphics\Icons\nsis1-uninstall.ico"
UninstallIcon "${NSISDIR}\Contrib\Graphics\Icons\lc.ico"

Section "Uninstall"

  DeleteRegKey HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\LC2JAVA_HOME"
  DeleteRegKey HKLM "SOFTWARE\WWW.LETZTECHANCE.ORG\LC2JAVA_HOME"

;  Delete "$SMPROGRAMS\WWW.LETZTECHANCE.ORG\LC2JAVA_HOME\reg\*.*"
...
;  File /a "exe\lc2sys.dll"
  Delete "$INSTDIR\LC2JAVA_HOMEuninst.exe"
  Delete "$INSTDIR\*.*"
  RMDir "$INSTDIR\"
  
  Delete "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2JAVA_HOME\LC2JAVA_HOME.lnk"
  Delete "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2JAVA_HOME\LC2JAVA_HOMEuninst.lnk"
  Delete "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2JAVA_HOME\Support.lnk"
