...
;Version Information
  ...
  VIAddVersionKey /LANG=${LANG_ENGLISH} "ProductName" "LC2ITunes.Export"
  ...
  VIAddVersionKey /LANG=${LANG_ENGLISH} "FileDescription" "LC2ITunes.Export"
  VIAddVersionKey /LANG=${LANG_ENGLISH} "FileVersion" "1.2.3"

;--------------------------------


Name "LC2ITunes.Export"
Caption "LC2ITunes.Export Install (c)by http://www.letztechance.org"
Icon "${NSISDIR}\Contrib\Graphics\Icons\lc.ico"
OutFile "LC2ITunes.Exportinstall.exe"
...
InstallDir "$PROGRAMFILES\LETZTECHANCE.ORG\LC2ITunes.Export"
InstallDirRegKey HKLM "Software\LETZTECHANCE.ORG\LC2ITunes.Export" "LC2ITunes.Export"
...
;	CreateShortCut "$SMPROGRAMS\$R0\All users LC2ITunes.Export.lnk" $INSTDIR\LC2ITunes.Export.exe
;Abort
;!else
...
;MessageBox MB_YESNO "Wollen Sie dieses Programm wirklich installieren ?" IDYES
  ...
  WriteRegStr HKLM SOFTWARE\WWW.LETZTECHANCE.ORG\LC2ITunes.Export "Install_Dir" "$INSTDIR"

  ; write uninstall strings
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\LC2ITunes.Export" "DisplayName" "LC2ITunes.Export (remove only)"
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\LC2ITunes.Export" "UninstallString" '"$INSTDIR\LC2ITunes.Exportuninst.exe"'
;---Extrahiere Dateien --------------------->
  SetOutPath $INSTDIR
  ;File /a "exe\LC2ITunes.Export.exe"
  File /a "exe\*.*"
...
;  File /a "exe\WebSearch\*.*"
  ...
  WriteUninstaller "LC2ITunes.Exportuninst.exe"
;---Extrahiere Bilder --------------------->
...
SectionIn 1 4 3
  ...
  WriteRegStr HKLM SOFTWARE\LETZTECHANCE.ORG\LC2ITunes.Export "StrTest_INSTDIR" "$INSTDIR"
  WriteRegDword HKLM SOFTWARE\LETZTECHANCE.ORG\LC2ITunes.Export "DwordTest_0xDEADBEEF" 0xdeadbeef
  WriteRegDword HKLM SOFTWARE\LETZTECHANCE.ORG\LC2ITunes.Export "DwordTest_123456" 123456
  WriteRegDword HKLM SOFTWARE\LETZTECHANCE.ORG\LC2ITunes.Export "DwordTest_0123" 0123
  WriteRegBin HKLM SOFTWARE\LETZTECHANCE.ORG\LC2ITunes.Export "BinTest_deadbeef01f00dbeef" "DEADBEEF01F00DBEEF"
;  StrCpy $8 "$SYSDIR\LC"
...
Section "CreateShortCuts"
 ...
 CreateDirectory $SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2ITunes.Export
 ;CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2ITunes.Export\Start LC2ITunes.Export.lnk" $INSTDIR\start.bat
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2ITunes.Export\LC2ITunes.Export.lnk" $INSTDIR\LC2ITunes.Export.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2ITunes.Export\Update.lnk" $INSTDIR\LC2Update.exe
 ;CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2ITunes.Export\LC2ITunes.Export.exe" $INSTDIR\LC2ITunes.Export.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2ITunes.Export\LC2ITunes.Exportuninst.lnk" $INSTDIR\LC2ITunes.Exportuninst.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2ITunes.Export\Support.lnk" $INSTDIR\letztechance.org.url
  SectionIn 1 2 3
...
Section "Programm und Hilfe ausführen..." TESTIDX
  ...
  SearchPath $1 $INSTDIR\LC2ITunes.Export.exe
...
UninstallText "Uninstall LC2ITunes.Export ?"
;UninstallIcon "${NSISDIR}\Contrib\Graphics\Icons\nsis1-uninstall.ico"
UninstallIcon "${NSISDIR}\Contrib\Graphics\Icons\lc.ico"

Section "Uninstall"

  DeleteRegKey HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\LC2ITunes.Export"
  DeleteRegKey HKLM "SOFTWARE\WWW.LETZTECHANCE.ORG\LC2ITunes.Export"

;  Delete "$SMPROGRAMS\WWW.LETZTECHANCE.ORG\LC2ITunes.Export\reg\*.*"
...
;  File /a "exe\lc2sys.dll"
  Delete "$INSTDIR\LC2ITunes.Exportuninst.exe"
  Delete "$INSTDIR\*.*"
  RMDir "$INSTDIR\"
  
  Delete "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2ITunes.Export\LC2ITunes.Export.lnk"
  Delete "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2ITunes.Export\LC2ITunes.Exportuninst.lnk"
  Delete "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2ITunes.Export\Support.lnk"
