...
;Version Information
  ...
  VIAddVersionKey /LANG=${LANG_ENGLISH} "ProductName" "LC2IEHistory"
  ...
  VIAddVersionKey /LANG=${LANG_ENGLISH} "FileDescription" "LC2IEHistory"
...
Name "LC2IEHistory"
Caption "LC2IEHistory Install (c)by http://www.letztechance.org"
Icon "${NSISDIR}\Contrib\Graphics\Icons\lc.ico"
OutFile "LC2IEHistoryinstall.exe"
...
InstallDir "$PROGRAMFILES\LETZTECHANCE.ORG\LC2IEHistory"
InstallDirRegKey HKLM "Software\LETZTECHANCE.ORG\LC2IEHistory" "LC2IEHistory"
...
;	CreateShortCut "$SMPROGRAMS\$R0\All users LC2IEHistory.lnk" $INSTDIR\LC2IEHistory.exe
;Abort
;!else
...
;MessageBox MB_YESNO "Wollen Sie dieses Programm wirklich installieren ?" IDYES
  ...
  WriteRegStr HKLM SOFTWARE\WWW.LETZTECHANCE.ORG\LC2IEHistory "Install_Dir" "$INSTDIR"

  ; write uninstall strings
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\LC2IEHistory" "DisplayName" "LC2IEHistory (remove only)"
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\LC2IEHistory" "UninstallString" '"$INSTDIR\LC2IEHistoryuninst.exe"'
;---Extrahiere Dateien --------------------->
  SetOutPath $INSTDIR
  ;File /a "exe\LC2IEHistory.exe"
  File /a "exe\*.*"
...
;  File /a "exe\WebSearch\*.*"
  ...
  WriteUninstaller "LC2IEHistoryuninst.exe"
;---Extrahiere Bilder --------------------->
...
SectionIn 1 4 3
  ...
  WriteRegStr HKLM SOFTWARE\LETZTECHANCE.ORG\LC2IEHistory "StrTest_INSTDIR" "$INSTDIR"
  WriteRegDword HKLM SOFTWARE\LETZTECHANCE.ORG\LC2IEHistory "DwordTest_0xDEADBEEF" 0xdeadbeef
  WriteRegDword HKLM SOFTWARE\LETZTECHANCE.ORG\LC2IEHistory "DwordTest_123456" 123456
  WriteRegDword HKLM SOFTWARE\LETZTECHANCE.ORG\LC2IEHistory "DwordTest_0123" 0123
  WriteRegBin HKLM SOFTWARE\LETZTECHANCE.ORG\LC2IEHistory "BinTest_deadbeef01f00dbeef" "DEADBEEF01F00DBEEF"
;  StrCpy $8 "$SYSDIR\LC"
...
Section "CreateShortCuts"
 ...
 CreateDirectory $SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2IEHistory
 ;CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2IEHistory\Start LC2IEHistory.lnk" $INSTDIR\start.bat
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2IEHistory\LC2IEHistory.lnk" $INSTDIR\LC2IEHistory.exe
 ;CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2IEHistory\LC2IEHistory.exe" $INSTDIR\LC2IEHistory.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2IEHistory\LC2IEHistoryuninst.lnk" $INSTDIR\LC2IEHistoryuninst.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2IEHistory\Support.lnk" $INSTDIR\letztechance.org.url
  SectionIn 1 2 3
...
Section "Programm und Hilfe ausführen..." TESTIDX
  ...
  SearchPath $1 $INSTDIR\LC2IEHistory.exe
...
UninstallText "Uninstall LC2IEHistory ?"
;UninstallIcon "${NSISDIR}\Contrib\Graphics\Icons\nsis1-uninstall.ico"
UninstallIcon "${NSISDIR}\Contrib\Graphics\Icons\lc.ico"

Section "Uninstall"

  DeleteRegKey HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\LC2IEHistory"
  DeleteRegKey HKLM "SOFTWARE\WWW.LETZTECHANCE.ORG\LC2IEHistory"

;  Delete "$SMPROGRAMS\WWW.LETZTECHANCE.ORG\LC2IEHistory\reg\*.*"
...
;  File /a "exe\lc2sys.dll"
  Delete "$INSTDIR\LC2IEHistoryuninst.exe"
  Delete "$INSTDIR\*.*"
  RMDir "$INSTDIR\"
  
  Delete "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2IEHistory\LC2IEHistory.lnk"
  Delete "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2IEHistory\LC2IEHistoryuninst.lnk"
  Delete "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2IEHistory\Support.lnk"
