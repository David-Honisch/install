...
!define AppName "LC2GrabURL Professional"
!define AppVersion "1.0"
!define ShortName "LC2GrabURL"
!define JRE_VERSION "1.4.1"
!define Vendor "Letztechance.org"
...
;Configuration
  ...
  ;General
  Name "${AppName}"
  OutFile "lc2graburl.exe"
 
  ;Folder selection page
  InstallDir "$PROGRAMFILES\${Vendor}\${SHORTNAME}"
  
  ;Get install folder from registry if available
  ;InstallDirRegKey HKLM "SOFTWARE\${Vendor}\${ShortName}" ""

  ;Get installation folder from registry if available
  InstallDirRegKey HKCU "Software\${Vendor}\${SHORTNAME}" ""
...
Section "Start Menu Shortcuts"
  CreateDirectory "$SMPROGRAMS\${Vendor}\${SHORTNAME}"
  CreateShortCut "$SMPROGRAMS\${Vendor}\${SHORTNAME}\Uninstall.lnk" "$INSTDIR\uninstall.exe" "" "$INSTDIR\uninstall.exe"
  CreateShortCut "$SMPROGRAMS\${Vendor}\${SHORTNAME}\${SHORTNAME}.lnk" "$INSTDIR\${SHORTNAME}.exe" "" "$INSTDIR\lc.ico"
SectionEnd
...
;Modern UI Configuration
  ...
  !define MUI_HEADERIMAGE "d:\install\Splash.bmp"
  !define MUI_HEADERIMAGE_BITMAP_NOSTRETCH
  !define MUI_HEADERIMAGE_BITMAP "D:\install\Splash.bmp"
  !define MUI_ICON "D:\install\lc.ico"
  !define MUI_UNICON "D:\install\lc.ico"
  !define MUI_TEXT "LC2GrabURL"
...
;Pages
  ...
  ; License page
  ;!insertmacro MUI_PAGE_LICENSE "D:\install\licence\License.txt"
...
;Language Strings
  ...
  ;Description
  LangString DESC_SecAppFiles ${LANG_ENGLISH} "LC2GrabURL files copy"
...
;  File /oname=$TEMP\jre_setup.exe D:\install\j2re-setup.exe
...
Section "Installation of ${AppName}" SecAppFiles
 ...
 File "D:\install\lc2graburl\lc2url.exe"
 File "D:\install\lc2graburl\lc2sys.dll"
 File "D:\install\lc2graburl\PageGrabber.class"
 File "D:\install\lc.ico"
;File /r "stream\"
...
Section "Start menu shortcuts" SecCreateShortcut
  ...
  CreateDirectory "$SMPROGRAMS\${Vendor}\${AppName}"
  CreateShortCut "$SMPROGRAMS\${Vendor}\${AppName}\Uninstall.lnk" "$INSTDIR\uninstall.exe" "" "$INSTDIR\uninstall.exe" 0
...
FoundOld:
  ...
  !insertmacro MUI_INSTALLOPTIONS_WRITE "D:\install\jre.ini" "Field 1" "Text" "${AppName} requires the Java Runtime Environment than the one found. The installation of JRE ${JRE_VERSION} will start."
...
Section "Uninstall"
  ...
  Delete "$SMPROGRAMS\${Vendor}\${AppName}\*.*"
  ; remove instdir.
  Delete "$SMPROGRAMS\${Vendor}\${ShortName}\*.*"
  ; remove files
  RMDir /r "$INSTDIR"
