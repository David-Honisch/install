...
;Version Information
  ...
  VIAddVersionKey /LANG=${LANG_ENGLISH} "ProductName" "LC2GrabUrls"
  ...
  VIAddVersionKey /LANG=${LANG_ENGLISH} "FileDescription" "LC2GrabUrls"
...
Name "LC2GrabUrls"
Caption "LC2GrabUrls Install (c)by http://www.letztechance.org"
Icon "${NSISDIR}\Contrib\Graphics\Icons\lc.ico"
OutFile "LC2GrabUrlsinstall.exe"
...
InstallDir "$PROGRAMFILES\LETZTECHANCE.ORG\LC2GrabUrls"
InstallDirRegKey HKLM "Software\LETZTECHANCE.ORG\LC2GrabUrls" "LC2GrabUrls"
...
;	CreateShortCut "$SMPROGRAMS\$R0\All users LC2GrabUrls.lnk" $INSTDIR\LC2GrabUrls.exe
;Abort
;!else
...
;MessageBox MB_YESNO "Wollen Sie dieses Programm wirklich installieren ?" IDYES
  ...
  WriteRegStr HKLM SOFTWARE\WWW.LETZTECHANCE.ORG\LC2GrabUrls "Install_Dir" "$INSTDIR"

  ; write uninstall strings
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\LC2GrabUrls" "DisplayName" "LC2GrabUrls (remove only)"
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\LC2GrabUrls" "UninstallString" '"$INSTDIR\LC2GrabUrlsuninst.exe"'
;---Extrahiere Dateien --------------------->
  SetOutPath $INSTDIR
  ;File /a "exe\LC2GrabUrls.exe"
  File /a "exe\*.*"
...
;---Extrahiere HTML --------------------->
  ...
  WriteUninstaller "LC2GrabUrlsuninst.exe"
;---Extrahiere Bilder --------------------->
...
SectionIn 1 4 3
  ...
  WriteRegStr HKLM SOFTWARE\LETZTECHANCE.ORG\LC2GrabUrls "StrTest_INSTDIR" "$INSTDIR"
  WriteRegDword HKLM SOFTWARE\LETZTECHANCE.ORG\LC2GrabUrls "DwordTest_0xDEADBEEF" 0xdeadbeef
  WriteRegDword HKLM SOFTWARE\LETZTECHANCE.ORG\LC2GrabUrls "DwordTest_123456" 123456
  WriteRegDword HKLM SOFTWARE\LETZTECHANCE.ORG\LC2GrabUrls "DwordTest_0123" 0123
  WriteRegBin HKLM SOFTWARE\LETZTECHANCE.ORG\LC2GrabUrls "BinTest_deadbeef01f00dbeef" "DEADBEEF01F00DBEEF"
;  StrCpy $8 "$SYSDIR\LC"
...
Section "CreateShortCuts"
 ...
 CreateDirectory $SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2GrabUrls
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2GrabUrls\Start LC2GrabUrls.lnk" $INSTDIR\start.bat
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2GrabUrls\LC2GrabUrls.lnk" $INSTDIR\LC2GrabUrls.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2GrabUrls\LC2GrabUrlsuninst.lnk" $INSTDIR\LC2GrabUrlsuninst.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2GrabUrls\Support.lnk" $INSTDIR\letztechance.org.url
  SectionIn 1 2 3
...
Section "Programm und Hilfe ausführen..." TESTIDX
  ...
  SearchPath $1 $INSTDIR\LC2GrabUrls.exe
...
UninstallText "Uninstall LC2GrabUrls ?"
;UninstallIcon "${NSISDIR}\Contrib\Graphics\Icons\nsis1-uninstall.ico"
UninstallIcon "${NSISDIR}\Contrib\Graphics\Icons\lc.ico"

Section "Uninstall"

  DeleteRegKey HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\LC2GrabUrls"
  DeleteRegKey HKLM "SOFTWARE\WWW.LETZTECHANCE.ORG\LC2GrabUrls"

;  Delete "$SMPROGRAMS\WWW.LETZTECHANCE.ORG\LC2GrabUrls\reg\*.*"
...
;  File /a "exe\lc2sys.dll"
  Delete "$INSTDIR\LC2GrabUrlsuninst.exe"
  Delete "$INSTDIR\*.*"
  RMDir "$INSTDIR\"
  
  Delete "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2GrabUrls\LC2GrabUrls.lnk"
  Delete "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2GrabUrls\LC2GrabUrlsuninst.lnk"
  Delete "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2GrabUrls\Support.lnk"
