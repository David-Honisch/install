...
;Version Information
  ...
  VIAddVersionKey /LANG=${LANG_ENGLISH} "ProductName" "LC2Paint"
  VIAddVersionKey /LANG=${LANG_ENGLISH} "Comments" "LC2Paint Client"
  ...
  VIAddVersionKey /LANG=${LANG_ENGLISH} "FileDescription" "LC2Paint"
...
Name "LC2Paint"
Caption "LC2Paint Install (c)by http://www.letztechance.org"
Icon "${NSISDIR}\Contrib\Graphics\Icons\lc.ico"
OutFile "LC2Paintinstall.exe"
...
InstallDir "$PROGRAMFILES\LETZTECHANCE.ORG\LC2Paint"
InstallDirRegKey HKLM "Software\LETZTECHANCE.ORG\LC2Paint" "LC2Paint"
...
;	CreateShortCut "$SMPROGRAMS\$R0\All users LC2Paint.lnk" $INSTDIR\LC2Paint.exe
;Abort
;!else
...
;MessageBox MB_YESNO "Wollen Sie dieses Programm wirklich installieren ?" IDYES
  ...
  WriteRegStr HKLM SOFTWARE\WWW.LETZTECHANCE.ORG\LC2Paint "Install_Dir" "$INSTDIR"

  ; write uninstall strings
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\LC2Paint" "DisplayName" "LC2Paint (remove only)"
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\LC2Paint" "UninstallString" '"$INSTDIR\uninst.exe"'
;---Extrahiere Dateien --------------------->
  SetOutPath $INSTDIR
  ;File /a "exe\LC2Paint.exe"
  File /a "exe\*.*"
...
;---EOF Config --------------------->  
;---Extrahiere Daten --------------------->
  CreateDirectory "$INSTDIR\data\img\" ; 2 recursively create a directory for fun.
  SetOutPath $INSTDIR\data\img
  File /a "exe\data\img\*.*"  
;---Extrahiere Config Plugins EOF--------------------->
;---Extrahiere Config Plugins --------------------->
  CreateDirectory "$INSTDIR\data\img\" ; 2 recursively create a directory for fun.
  SetOutPath $INSTDIR\data\audio
  File /a "exe\data\audio\*.*"  
...
;---Extrahiere Plugins --------------------->
  CreateDirectory "$INSTDIR\shared" ; 2 recursively create a directory for fun.
...
SectionIn 1 4 3
  ...
  WriteRegStr HKLM SOFTWARE\LETZTECHANCE.ORG\LC2Paint "StrTest_INSTDIR" "$INSTDIR"
  WriteRegDword HKLM SOFTWARE\LETZTECHANCE.ORG\LC2Paint "DwordTest_0xDEADBEEF" 0xdeadbeef
  WriteRegDword HKLM SOFTWARE\LETZTECHANCE.ORG\LC2Paint "DwordTest_123456" 123456
  WriteRegDword HKLM SOFTWARE\LETZTECHANCE.ORG\LC2Paint "DwordTest_0123" 0123
  WriteRegBin HKLM SOFTWARE\LETZTECHANCE.ORG\LC2Paint "BinTest_deadbeef01f00dbeef" "DEADBEEF01F00DBEEF"
;  StrCpy $8 "$SYSDIR\LC"
...
Section "CreateShortCuts"
 ...
 CreateDirectory $SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2Paint
 ;CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2Paint\Start LC2Paint.lnk" $INSTDIR\start.bat
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2Paint\LC2Paint.lnk" $INSTDIR\launcher.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2Paint\About.lnk" $INSTDIR\about.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2Paint\Uninstall.lnk" $INSTDIR\uninst.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2Paint\Support.lnk" $INSTDIR\letztechance.org.url
  SectionIn 1 2 3
...
Section "Programm und Hilfe ausführen..." TESTIDX
  ...
  SearchPath $1 $INSTDIR\launcher.exe
...
UninstallText "Uninstall LC2Paint ?"
;UninstallIcon "${NSISDIR}\Contrib\Graphics\Icons\nsis1-uninstall.ico"
UninstallIcon "${NSISDIR}\Contrib\Graphics\Icons\lc.ico"

Section "Uninstall"

  DeleteRegKey HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\LC2Paint"
  DeleteRegKey HKLM "SOFTWARE\WWW.LETZTECHANCE.ORG\LC2Paint"

;  Delete "$SMPROGRAMS\WWW.LETZTECHANCE.ORG\LC2Paint\reg\*.*"
...
;  File /a "exe\lc2sys.dll"
  ...
  Delete "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2Paint\LC2Paint.lnk"
  Delete "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2Paint\uninst.lnk"
  Delete "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2Paint\*.*"
