...
;Version Information
  ...
  VIAddVersionKey /LANG=${LANG_ENGLISH} "ProductName" "LC2MonoTorrent.GUI"
  ...
  VIAddVersionKey /LANG=${LANG_ENGLISH} "FileDescription" "LC2MonoTorrent.GUI"
  VIAddVersionKey /LANG=${LANG_ENGLISH} "FileVersion" "1.2.3"

;--------------------------------


Name "LC2MonoTorrent.GUI"
Caption "LC2MonoTorrent.GUI Install (c)by http://www.letztechance.org"
Icon "${NSISDIR}\Contrib\Graphics\Icons\lc.ico"
OutFile "LC2MonoTorrent.GUIinstall.exe"
...
InstallDir "$PROGRAMFILES\LETZTECHANCE.ORG\LC2MonoTorrent.GUI"
InstallDirRegKey HKLM "Software\LETZTECHANCE.ORG\LC2MonoTorrent.GUI" "LC2MonoTorrent.GUI"
...
;	CreateShortCut "$SMPROGRAMS\$R0\All users LC2MonoTorrent.GUI.lnk" $INSTDIR\LC2MonoTorrent.GUI.exe
;Abort
;!else
...
;MessageBox MB_YESNO "Wollen Sie dieses Programm wirklich installieren ?" IDYES
  ...
  WriteRegStr HKLM SOFTWARE\WWW.LETZTECHANCE.ORG\LC2MonoTorrent.GUI "Install_Dir" "$INSTDIR"

  ; write uninstall strings
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\LC2MonoTorrent.GUI" "DisplayName" "LC2MonoTorrent.GUI (remove only)"
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\LC2MonoTorrent.GUI" "UninstallString" '"$INSTDIR\LC2MonoTorrent.GUIuninst.exe"'
;---Extrahiere Dateien --------------------->
  SetOutPath $INSTDIR
  ;File /a "exe\LC2MonoTorrent.GUI.exe"
  File /a "exe\*.*"
...
;  File /a "exe\WebSearch\*.*"
  ...
  WriteUninstaller "LC2MonoTorrent.GUIuninst.exe"
;---Extrahiere Bilder --------------------->
...
SectionIn 1 4 3
  ...
  WriteRegStr HKLM SOFTWARE\LETZTECHANCE.ORG\LC2MonoTorrent.GUI "StrTest_INSTDIR" "$INSTDIR"
  WriteRegDword HKLM SOFTWARE\LETZTECHANCE.ORG\LC2MonoTorrent.GUI "DwordTest_0xDEADBEEF" 0xdeadbeef
  WriteRegDword HKLM SOFTWARE\LETZTECHANCE.ORG\LC2MonoTorrent.GUI "DwordTest_123456" 123456
  WriteRegDword HKLM SOFTWARE\LETZTECHANCE.ORG\LC2MonoTorrent.GUI "DwordTest_0123" 0123
  WriteRegBin HKLM SOFTWARE\LETZTECHANCE.ORG\LC2MonoTorrent.GUI "BinTest_deadbeef01f00dbeef" "DEADBEEF01F00DBEEF"
;  StrCpy $8 "$SYSDIR\LC"
...
Section "CreateShortCuts"
 ...
 CreateDirectory $SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2MonoTorrent.GUI
 ;CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2MonoTorrent.GUI\Start LC2MonoTorrent.GUI.lnk" $INSTDIR\start.bat
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2MonoTorrent.GUI\LC2MonoTorrent.GUI.lnk" $INSTDIR\LC2MonoTorrent.GUI.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2MonoTorrent.GUI\Update.lnk" $INSTDIR\lc2update.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2MonoTorrent.GUI\LC2MonoTorrent.GUIuninst.lnk" $INSTDIR\LC2MonoTorrent.GUIuninst.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2MonoTorrent.GUI\Support.lnk" $INSTDIR\letztechance.org.url
  SectionIn 1 2 3
...
Section "Programm und Hilfe ausführen..." TESTIDX
  ...
  SearchPath $1 $INSTDIR\LC2MonoTorrent.GUI.exe
...
UninstallText "Uninstall LC2MonoTorrent.GUI ?"
;UninstallIcon "${NSISDIR}\Contrib\Graphics\Icons\nsis1-uninstall.ico"
UninstallIcon "${NSISDIR}\Contrib\Graphics\Icons\lc.ico"

Section "Uninstall"

  DeleteRegKey HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\LC2MonoTorrent.GUI"
  DeleteRegKey HKLM "SOFTWARE\WWW.LETZTECHANCE.ORG\LC2MonoTorrent.GUI"

;  Delete "$SMPROGRAMS\WWW.LETZTECHANCE.ORG\LC2MonoTorrent.GUI\reg\*.*"
...
;  File /a "exe\lc2sys.dll"
  Delete "$INSTDIR\LC2MonoTorrent.GUIuninst.exe"
  Delete "$INSTDIR\*.*"
  RMDir "$INSTDIR\"
  
  Delete "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2MonoTorrent.GUI\LC2MonoTorrent.GUI.lnk"
  Delete "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2MonoTorrent.GUI\LC2MonoTorrent.GUIuninst.lnk"
  Delete "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2MonoTorrent.GUI\Support.lnk"
