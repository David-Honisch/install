...
;Version Information
  ...
  VIAddVersionKey /LANG=${LANG_ENGLISH} "ProductName" "LC2Launcher.NET"
  VIAddVersionKey /LANG=${LANG_ENGLISH} "Comments" "LC2Launcher.NET Client"
  ...
  VIAddVersionKey /LANG=${LANG_ENGLISH} "FileDescription" "LC2Launcher.NET"
  VIAddVersionKey /LANG=${LANG_ENGLISH} "FileVersion" "1.0.0"

;--------------------------------


Name "LC2Launcher.NET"
Caption "LC2Launcher.NET Install (c)by http://www.letztechance.org"
Icon "${NSISDIR}\Contrib\Graphics\Icons\lc.ico"
OutFile "LC2Launcher.NETinstall.exe"
...
InstallDir "$PROGRAMFILES\LETZTECHANCE.ORG\LC2Launcher.NET"
InstallDirRegKey HKLM "Software\LETZTECHANCE.ORG\LC2Launcher.NET" "LC2Launcher.NET"
...
;	CreateShortCut "$SMPROGRAMS\$R0\All users LC2Launcher.NET.lnk" $INSTDIR\LC2Launcher.NET.exe
;Abort
;!else
...
;MessageBox MB_YESNO "Wollen Sie dieses Programm wirklich installieren ?" IDYES
  ...
  WriteRegStr HKLM SOFTWARE\WWW.LETZTECHANCE.ORG\LC2Launcher.NET "Install_Dir" "$INSTDIR"

  ; write uninstall strings
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\LC2Launcher.NET" "DisplayName" "LC2Launcher.NET (remove only)"
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\LC2Launcher.NET" "UninstallString" '"$INSTDIR\uninst.exe"'
;---Extrahiere Dateien --------------------->
  SetOutPath $INSTDIR
  ;File /a "exe\LC2Launcher.NET.exe"
  ;File /a "exe\*.*"
  File /a "..\install\*.*"
...
SectionIn 1 4 3
  ...
  WriteRegStr HKLM SOFTWARE\LETZTECHANCE.ORG\LC2Launcher.NET "StrTest_INSTDIR" "$INSTDIR"
  WriteRegDword HKLM SOFTWARE\LETZTECHANCE.ORG\LC2Launcher.NET "DwordTest_0xDEADBEEF" 0xdeadbeef
  WriteRegDword HKLM SOFTWARE\LETZTECHANCE.ORG\LC2Launcher.NET "DwordTest_123456" 123456
  WriteRegDword HKLM SOFTWARE\LETZTECHANCE.ORG\LC2Launcher.NET "DwordTest_0123" 0123
  WriteRegBin HKLM SOFTWARE\LETZTECHANCE.ORG\LC2Launcher.NET "BinTest_deadbeef01f00dbeef" "DEADBEEF01F00DBEEF"
;  StrCpy $8 "$SYSDIR\LC"
...
Section "CreateShortCuts"
 ...
 CreateDirectory $SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2Launcher.NET
 ;CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2Launcher.NET\Start LC2Launcher.NET.lnk" $INSTDIR\start.bat
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2Launcher.NET\LC2Launcher.NET.lnk" $INSTDIR\LC2Start.NET.exe
 CreateShortCut "$INSTDIR\launcher.lnk" $INSTDIR\LC2Start.NET.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2Launcher.NET\AutoStart.lnk" $INSTDIR\LC2autostart.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2Launcher.NET\AutoStartConfig.lnk" $INSTDIR\LC2autostartConfig.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2Launcher.NET\AutoStartUninstall.lnk" $INSTDIR\LC2autostartUninstall.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2Launcher.NET\LC2Config.lnk" $INSTDIR\LC2Config.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2Launcher.NET\Uninstall.lnk" $INSTDIR\uninst.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2Launcher.NET\Support.lnk" $INSTDIR\letztechance.org.url
  SectionIn 1 2 3
...
UninstallText "Uninstall LC2Launcher.NET ?"
;UninstallIcon "${NSISDIR}\Contrib\Graphics\Icons\nsis1-uninstall.ico"
UninstallIcon "${NSISDIR}\Contrib\Graphics\Icons\lc.ico"

Section "Uninstall"

  DeleteRegKey HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\LC2Launcher.NET"
  DeleteRegKey HKLM "SOFTWARE\WWW.LETZTECHANCE.ORG\LC2Launcher.NET"

;  Delete "$SMPROGRAMS\WWW.LETZTECHANCE.ORG\LC2Launcher.NET\reg\*.*"
...
;  File /a "exe\lc2sys.dll"
  ...
  Delete "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2Launcher.NET\LC2Launcher.NET.lnk"
  Delete "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2Launcher.NET\uninst.lnk"
  Delete "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2Launcher.NET\*.*"
